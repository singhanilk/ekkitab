...
my $authorcol = -1;
 ...
 if (($availcol >= 0) && ($pricecol >= 0) && ($isbncol >= 0) && ($imprintcol >= 0) 
			  && ($titlecol >= 0) && ($authorcol >= 0)) {
    $startrow = $iR + 1;
    $endrow   = $oWkS->{MaxRow};
    last;
 }
...
for (my $i = $startrow; $i <= $endrow; $i++) {
    ...
    if (defined ($isbn) && defined ($price) && defined ($currency) && defined ($availability) && defined ($imprint)
	&& defined ($title) && defined ($author)){
