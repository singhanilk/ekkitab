...
sub writeBookDetails {
    ...
    #}
    if (($title eq "") or ($author eq "") or ($description eq "")) {
       $icount++;
    }
    return $line;
}
...
sub main {
    open (FILE, "penguinindia_books.txt") or die "Cannot open input file: ";
    my $ua = initLWP();
    open (OUTPUT, ">>", "penguinindia_bookdetail-final.txt") or die "Cannot open output file"; 
    OUTPUT->autoflush(1);
    my $i = 0;
    while (my $line = <FILE>) {
      chomp($line);
      ...
      if (($i % 10) == 0) {
        print "Completed $i books...[$icount incomplete].\n";
      }
    }
    print "Completed $i books. Information for [$icount] books incomplete.\n";
    close(FILE);
    close(OUTPUT);
}
$icount = 0;
