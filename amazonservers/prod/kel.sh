# large  instance : original  image from canonical  
date
...
amiid=ami-7d43ae14  # Ubuntu 9.10 , 64-bit, EBS boot in that region


instanceid=$(ec2-run-instances  $amiid --group $group  --key $keypair   --region $region   --availability-zone $zone   --instance-type $type    -b /dev/sda1=:20 -b /dev/sdb=ephemeral0 -b /dev/sdc=ephemeral1 | egrep ^INSTANCE | cut -f2)
